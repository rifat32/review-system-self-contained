...
PROMPT;
    ...
    public function generateRollingInsight(?array $previousInsight, array $latestInsight, array $currentMetrics = []): array
    {
        ...
        $systemPrompt = "You are an AI that updates an existing structured business intelligence report.\n\n"
            ...
            . "The second input contains ONLY new reviews received since the last update. Each review includes: rating, sentiment, emotion, topics, positive_aspects, negative_aspects, issues, summary, confidence, recommendations, abusive_language, spam_probability, and sarcasm.\n"
            . "The third input contains business metrics, backend-calculated statistics (issue frequencies, topic counts, emotion frequencies, rule trigger counts), and trend inputs (average rating delta, negative percentage delta).\n"
            . "Update the business intelligence state using ALL three inputs:\n"
            . "- Use the provided 'trend_inputs' (previous vs current average rating and negative review percentage change) as the direct ground truth for trend direction. If the difference is negative or negative percentage increased, explain this shift.\n"
            . "- Prioritize new reviews that have higher 'confidence' scores. Low-confidence reviews (e.g., confidence < 0.7) should have significantly less weight on your analysis.\n"
            . "- Use the backend-calculated frequencies (issues, topics, emotions, and local rule triggers) to detect recurring problems rather than guessing from summaries alone.\n"
            . "- When updating the trend direction, consider both the previous review count and the new batch size. Avoid drastic trend changes unless supported by the trend inputs.\n"
            . "- If trends are changing, identify the exact reasons based on metrics (e.g., waiting time complaints increased, average rating dropped) and output them in the 'trend_reasons' array.\n"
            . "- Evolve the summary naturally, incorporating the current metrics where helpful.\n"
            ...
            . "  \"trend_reasons\": [\"list of specific reasons explaining why the trend is Improving/Declining/Stable, e.g. 'Average rating dropped', 'Critical alerts increased'\"],\n"
