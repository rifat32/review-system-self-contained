  ...
  public static  function getStaffPerformance($branchId, $businessId, $startDate, $endDate, $limit = 5)
    {
        ...
        foreach ($staffReviews as $staffId => $reviews) {
            ...
            $staffPerformance[] = [
                ...
                'rating_out_of' => 5,
                'reviews_count' => $reviews->count(),
                'ai_evaluation' => self::getStaffEvaluation($avgRating, $reviews->count()),
                'has_profile' => true,
                'positive_percentage' => round(($reviews->where('sentiment_score', '>=', 0.7)->count() / $reviews->count()) * 100),
                'last_review_date' => $reviews->sortByDesc('created_at')->first()->created_at->diffForHumans()
            ];
   ...
   public static function getTopThreeStaff($businessId, $filters = [])
    {
        ...
        $staffPerformance = $staffGroups->map(function ($staffReviews, $staffId) {
            ...
            $topTopics = self::extractStaffTopics($staffReviews);
 ...
 public static   function getSubmissionsOverTime($reviews, $period)
    {
        ...
        $filledData = fillMissingPeriods($submissionsByPeriod, $startDate, $endDate, $groupFormat);
   ...
   public static  function getAllStaffMetricsFromReviewValue($reviews)
    {
        ...
        $staffMetrics = $staffGroups->map(function ($staffReviews, $staffId) {
            ...
            return [
                ...
                'position' => $staff->job_title ?? 'Staff',
                'avg_rating' => round($avgRating, 1),
                'sentiment_score' => self::getSentimentLabel($staffReviews->avg('sentiment_score')),
                'compliments_count' => $compliments,
                'complaints_count' => $complaints,
                'neutral_count' => $neutral,
